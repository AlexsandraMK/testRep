...
function chooseRequest(xhr, method, url, json_body, get_param) {
  xhr.open(method, 'http://26.109.231.110:8080' + url + get_param); //Белый сервак:194.87.95.63 Сашин:26.109.231.110
...
function prepareRequest(url){
  ...
  switch(url) {
    ...
    case '/add_pet':
      json = JSON.stringify({
        user_id : user_id,
        pet_name : "Дрыбла", //строка
        pet_gender : "2", //число - 1 (м), 2 (ж)
        breed_id : "1", //число - id породы breed_id
        pet_date_of_birth : "", //date
        pet_weight : "500", //число
        photos : "https://vetspravka.ru/imgs/blog/127/127/blg60acb726d9c9e6-25026911.jpg", //фото
        documents : "" //документы
      });
      method = "POST";
      break;
...
prepareRequest(url);
chooseRequest(xhr, method, url, json, get_param);
